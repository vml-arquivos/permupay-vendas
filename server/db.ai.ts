...
function parseContent(raw: unknown): string {
  ...
  if (Array.isArray(raw)) {
    return raw.map((part) => (part && typeof part === "object" && "text" in part ? String((part as { text?: unknown }).text ?? "") : "")).join("");
  }
  return "";
}

function normalizeSuggestion(value: unknown): ProductSuggestion {
...
export async function suggestProductInfo(input: { imageUrl?: string; name?: string }): Promise<ProductSuggestion> {
