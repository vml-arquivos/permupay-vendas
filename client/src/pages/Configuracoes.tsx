 ...
 *
 * Abas:
 * 1. Usuários — listar, criar, alterar role, ativar/desativar, redefinir senha
 * 2. Sistema — informações de banco, domínio, segurança
 * 3. Minha Conta — alterar nome e senha do usuário logado
 */
        ...
        <div className="border border-stone-200 rounded-xl p-5 bg-stone-50 space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full px-3 py-2 border border-stone-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-stone-400 pr-10"
                />
                <button
                  type="button"
          ...
          <div className="flex items-center gap-2 pt-1">
            <button
              ...
              disabled={createMutation.isPending || !form.email || !form.name || form.password.length < 8}
            ...
            <button
              ...
              className="px-4 py-2 border border-stone-300 hover:bg-stone-100 text-stone-700 text-sm font-medium rounded-lg transition-colors"
            >
              Cancelar
            </button>
          </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm space-y-4">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => resetPasswordMutation.mutate({ userId: resetTarget, newPassword })}
                disabled={resetPasswordMutation.isPending || newPassword.length < 8}
                className="flex-1 py-2 bg-stone-900 hover:bg-stone-700 disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
              >
              ...
              <button
                ...
                className="flex-1 py-2 border border-stone-300 hover:bg-stone-100 text-stone-700 text-sm font-medium rounded-lg transition-colors"
              >
                Cancelar
              </button>
            </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="border border-stone-200 rounded-xl p-4 space-y-2">
          ...
          <div className="flex items-center justify-between text-sm">
            <span className="text-stone-500">MySQL / TiDB</span>
            <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${
              systemQuery.data?.database?.connected
        ...
        <div className="border border-stone-200 rounded-xl p-4 space-y-2">
          <div className="flex items-center gap-2 text-stone-700">
            ...
            <span className="text-sm font-semibold">Armazenamento S3</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-stone-500">Upload de imagens</span>
            <span className={`inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full ${
              systemQuery.data?.storage?.configured
                ? "bg-emerald-100 text-emerald-700"
          ...
          {!systemQuery.data?.storage?.configured && (
            <p className="text-xs text-stone-400 leading-relaxed">
              Configure AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY e S3_BUCKET para habilitar uploads.
            </p>
          )}
        </div>
          ...
          <div className="space-y-1.5 text-sm">
            <div className="flex items-center justify-between">
              ...
              <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
                <CheckCircle2 className="w-3 h-3" />JWT + Cookie
              </span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-stone-500">Senhas</span>
              <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700">
                <CheckCircle2 className="w-3 h-3" />bcrypt (12 rounds)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="border border-stone-200 rounded-xl p-4 space-y-2">
          <div className="flex items-center gap-2 text-stone-700">
            ...
            <span className="text-sm font-semibold">Aplicação</span>
          </div>
          <div className="space-y-1.5 text-sm">
            <div className="flex items-center justify-between">
              <span className="text-stone-500">Plataforma</span>
              <span className="text-stone-700 text-xs font-medium">Permupay Vendas</span>
            </div>
            <div className="flex items-center justify-between">
    ...
    <div className="space-y-6 max-w-lg">
      ...
      <div className="border border-stone-200 rounded-xl p-5 space-y-3">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full bg-stone-900 flex items-center justify-center text-white font-bold text-lg">
        ...
        {showPasswordForm && (
          <div className="space-y-3 pt-1">
            ...
            <button
              ...
              disabled={resetMutation.isPending || passwords.newPass.length < 8 || passwords.newPass !== passwords.confirm}
              className="w-full py-2 bg-stone-900 hover:bg-stone-700 disabled:opacity-50 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2"
            >
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div>
          {activeTab === "usuarios" && isAdmin && user && (
            <TabUsuarios currentUserId={user.id} />
          )}
          {activeTab === "sistema" && isAdmin && (
            <TabSistema />
          )}
          {activeTab === "conta" && user && (
            <TabMinhaConta user={user} />
          )}
