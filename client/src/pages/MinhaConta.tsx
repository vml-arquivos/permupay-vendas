...
// Contato vindo do link (?contact=...) — ex.: depois de uma reserva feita
// pela vitrine — só serve para pré-preencher o campo de contato na tela de
// login/criar conta; a autenticação em si sempre depende da sessão real.
...
export default function MinhaConta() {
  ...
  if (meQuery.isLoading) {
    return (
      <main className="flex min-h-screen items-center justify-center bg-slate-50 text-sm text-muted-foreground">
        ...
        <div className="mx-auto max-w-md space-y-6">
          ...
          <header className="rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-7 text-white">
            ...
            <p className="mt-2 text-sm text-slate-300">
              Seus pedidos, recomendações e carrinho — protegidos por senha, porque guardam
              dados sigilosos (CPF, endereço, documentos).
            </p>
          </header>
  ...
  return <MinhaContaLogada customer={customer!} />;
}

function MinhaContaLogada({ customer }: { customer: SafeCustomer }) {
  const utils = trpc.useUtils();
  const cart = useCart();
  ...
  // Se a sessão trocar de cliente (ex.: logout + login com outra conta),
  // recarrega o formulário com os dados do novo cliente.
  useEffect(() => {
    setProfileForm({
      name: customer.name ?? "",
      email: customer.email ?? "",
    ...
    <main className="min-h-screen bg-slate-50 px-4 py-8 sm:px-6">
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <header className="rounded-3xl bg-gradient-to-br from-slate-950 via-slate-900 to-slate-800 p-7 text-white">
          ...
          <h1 className="mt-2 text-3xl font-bold">Olá, {customer.name.split(" ")[0]}</h1>
          <p className="mt-2 text-sm text-slate-300">
            Seu cadastro, histórico de compras, recomendações e carrinho — tudo em um só
            lugar, protegido pela sua conta.
          </p>
        </header>
            ...
            {!ordersQuery.isLoading && ordersQuery.data?.length === 0 && (
              <div className="rounded-2xl border bg-white p-10 text-center">
                ...
                <p className="mt-3 text-sm text-muted-foreground">
                  Você ainda não tem pedidos registrados com esta conta.
                </p>
              </div>
            )}
            {groups.length > 0 && (
            ...
            <div className="rounded-2xl border bg-white p-5 space-y-4">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input value={customer.contact} readOnly disabled />
                </div>
                <div className="space-y-2">
                  <Label>E-mail</Label>
                  <Input
